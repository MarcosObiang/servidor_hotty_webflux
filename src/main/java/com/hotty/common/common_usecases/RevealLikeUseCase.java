                ...
                return getLikeUseCase.execute(likeUID)
                                .onErrorResume(WebClientResponseException.class,
                                                ex -> Mono.error(new DownstreamServiceException(
                                                                "Error getting like from likes-service.",
                                ...
                                .flatMap(likeData -> {
                                        ...
                                        if (senderUID == null) {
                                                return Mono.error(new IllegalArgumentException(
                                                                "Like data is missing senderUID."));
                                        }

                                        if (likeData.getOfferExpirationDate().isBefore(Instant.now())) {
                                                purchaseType = "EXPIRED_REACTION_REVELATION";
                                        } else {
                                                purchaseType = "REACTION_REVELATION";
                                        }

                                        return makePurchaseForUserWithCreditsUseCase.execute(likeData.getReceiverUID(), purchaseType)
                                                        .onErrorResume(WebClientResponseException.class,
                                                                        ex -> Mono.error(
                                                                                        new DownstreamServiceException(
                                                                                                        "Error getting user data from user-service.",
                                                                                                        ex.getRawStatusCode(),
                                                                                                        ex.getResponseBodyAsString())))
                                                        .flatMap(userData -> {

                                                                String senderName = (String) userData.getName();
                                                                String senderPictureUrl = (String) userData
                                                                                .getUserImage1();

                                                                if (senderName == null || senderPictureUrl == null
                                                                                || userData.getBirthDate() == null) {
                                                                        return Mono.error(
                                                                                        new IllegalArgumentException(
                                                                                                        "User data from user-service is incomplete."));
                                                                }

                                                                // 3. Actualizar el like con la información revelada
                                                                return updateLikeUseCase
                                                                                .execute(likeUID, userUID, true,
                                                                                                senderPictureUrl,
                                                                                ...
                                                                                .onErrorResume(WebClientResponseException.class,
                                                                                                ex -> Mono.error(
                                                                                                                new DownstreamServiceException(
                                                                                                                                "Error updating like in likes-service.",
                                                                                                                                ex.getRawStatusCode(),
                                                                                                                                ex.getResponseBodyAsString())));
                                                        });
                                });
